			...
			<div class="tab-pane active" id="edit">
			    <?php echo form_open_multipart($this->uri->uri_string(), array('class' => 'form-horizontal form-bordered')); ?>
					...
					<div class="form-group">
						...
						<div class="col-md-6">
							<input type="text" class="form-control" name="patient_name" value="<?php echo set_value('patient_name', $testimonial['patient_name']); ?>" />
					...
					<div class="form-group ">
								...
								<div class="col-md-6">
									<input type="text" class="form-control" name="youtube_link" value="<?php echo $testimonial['youtube_link']; ?>"/>
								</div>
							</div>
					<div class="form-group">
					...
					<div class="form-group">
        						<label class="col-md-3 control-label"><?php echo translate('Page'); ?> <span class="required">*</span></label>
        						<div class="col-md-6">
        						    <select class="form-control" name="show_id" required>
        						        <?php   $this->db->select('staff.*,staff_designation.name as designation_name,staff_department.name as department_name,login_credential.role as role_id, roles.name as role');
                                                ...
                                                <option value="0" <?php if($testimonial['show_id'] == 0) echo "selected"; ?>>Home</option>
                                                <option value="user" <?php if($testimonial['show_id'] == "user") echo "selected"; ?>>User</option>
                                                <?php foreach($doctors as $doctor){?>
                                                <option value="<?=$doctor->id?>" <?php if($doctor->id == $testimonial['show_id']) echo "selected"; ?>><?=$doctor->name?></option>
                                                <?php } ?>
        						    </select>
        						</div>
        					</div>
					...
					<div class="form-group">
						...
						<div class="col-md-4">
							...
							<input type="file" name="photo" class="dropify" data-height="150" data-default-file="<?php echo $this->app_lib->get_image_url('testimonial/' . $testimonial['image']); ?>" />
