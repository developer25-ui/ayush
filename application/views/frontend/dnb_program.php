<section class="panel">
	<div class="tabs-custom">
		<ul class="nav nav-tabs">
			<li class="<?php echo (!isset($validation_error) ? 'active' : ''); ?>">
				<a href="#list" data-toggle="tab"><i class="fas fa-list-ul"></i> <?php echo translate('DNB Program') . " " . translate('list'); ?></a>
			</li>
	<?php if (get_permission('frontend_blogs', 'is_add')) { ?>
			<li class="<?php echo (isset($validation_error) ? 'active' : ''); ?>">
				<a href="#create" data-toggle="tab"><i class="far fa-edit"></i> <?php echo translate('add') . " " . translate('program'); ?></a>
			</li>
	<?php } ?>
				...
				<table class="table table-bordered table-hover table-condensed table_default" cellspacing="0" width="100%">
					<thead>
						<tr>
							...
							<th><?php echo translate('name_of_faculty'); ?></th>
						...
						<tr>
							...
							<td class="min-w-xs">
								<?php if (get_permission('frontend_blogs', 'is_edit')): ?>
									<a href="<?php echo base_url('frontend/content/dnb_program/edit/' . $row['id']); ?>" class="btn btn-default btn-circle icon" data-toggle="tooltip" data-original-title="<?php echo translate('edit'); ?>"> 
										<i class="fas fa-pen-nib"></i>
									</a>
								<?php endif; if (get_permission('frontend_blogs', 'is_delete')): ?>
									<?php echo btn_delete('frontend/content/dnb_program/delete/' . $row['id']); ?>
								<?php endif; ?>
							</td>
						</tr>
						<?php endforeach; }?>
			...
			<div class="tab-pane <?php echo (isset($validation_error) ? 'active' : ''); ?>" id="create">
			    <?php echo form_open_multipart($this->uri->uri_string(), array('class' => 'form-horizontal form-bordered')); ?>
					...
					<div class="form-group">
						...
						<div class="col-md-6">
							<input type="text" class="form-control" name="name_of_faculty"  />
						</div>
					</div>
					...
					<div class="form-group">
						...
						<div class="col-md-6">
							<input type="text" class="form-control" name="status_of_faculty"  />
						</div>
					</div>
					...
					<div class="form-group">
						...
						<div class="col-md-6">
							<input type="text" class="form-control" name="opd_timings"  />
						</div>
					</div>
	...
	<div class="tab-pane <?php echo ($validation == 6 ? 'active' : ''); ?>" id="options">
						<?php echo form_open($this->uri->uri_string(), array('class' => 'form-horizontal')); ?>
							<?php $home_seo = $this->db->get_where('dnb_program_seo',array('id'=>1))->row_array();
