...
class Home_model extends MY_Model
{
  ...
  public function get_doctor_profile_by_slug($slug)
    {
        $this->db->select('staff.*,front_cms_doctor_bio.biography,staff_designation.name as designation_name,staff_department.name as department_name,location.name as location_name');
        $this->db->from('staff');
        $this->db->join('staff_designation', 'staff_designation.id = staff.designation', 'left');
        $this->db->join('staff_department', 'staff_department.id = staff.department', 'left');
        $this->db->join('location', 'location.id = staff.location', 'left');
    ...
    public function get_doctor_profile_by_name($name)
    {
    ...
    public function get_doctor_list($start = '')
    {
        ...
        $this->db->order_by('staff.rank', 'asc');
        if ($start != '') {
     ...
     public function get_doctors_list($start = '')
    {
        $this->db->select('staff.*,staff_designation.name as designation_name,staff_department.name as department_name');
        $this->db->from('staff');
        $this->db->join('login_credential', 'login_credential.user_id = staff.id and login_credential.role != 7', 'inner');
    ...
    public function get_doctor_list_alphabetically($start = '')
    {
        $this->db->select('staff.*,SUBSTR(staff.name,5) as new_name,staff_designation.name as designation_name,staff_department.name as department_name');
        $this->db->from('staff');
        $this->db->join('login_credential', 'login_credential.user_id = staff.id and login_credential.role != 7', 'inner');
        $this->db->join('staff_designation', 'staff_designation.id = staff.designation', 'left');
        $this->db->join('staff_department', 'staff_department.id = staff.department', 'left');
    ...
    public function get_doctor_speciality_list($doctors)
    {
        $this->db->select('staff.*,staff_designation.name as designation_name,staff_department.name as department_name');
        $this->db->from('staff');
        $this->db->join('login_credential', 'login_credential.user_id = staff.id and login_credential.role != 7', 'inner');
        ...
        $this->db->where('login_credential.role', 3);
        $this->db->where('login_credential.active', 1);
        $this->db->where_in('staff.id', $doctors);
        $this->db->order_by('staff.id', 'asc');
        
        $result = $this->db->get()->result_array();
        return $result;
    }
public function get_doctor_location_list($doctors)
    {
        $this->db->select('staff.*,staff_designation.name as designation_name,staff_department.name as department_name');
        $this->db->from('staff');
        $this->db->join('login_credential', 'login_credential.user_id = staff.id and login_credential.role != 7', 'inner');
