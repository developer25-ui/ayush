...
class Prstoriess extends Admin_Controller
{

    public function __construct()
    {
        parent::__construct();
        $this->load->model('prstoriess_model');
    }
    ...
    public function index()
    {
        ...
        if (!get_permission('frontend_prstoriess', 'is_view')) {
            access_denied();
        }
        if($this->input->post('options')){
            $seo_content['page_title'] = $this->input->post('page_title');
                ...
                $this->db->where('id', 1);
                $this->db->update('front_cms_prstoriess_seo', $arraySeo);
                set_alert('success', translate('information_has_been_saved_successfully'));
                redirect(base_url('frontend/prstoriess'));
        }
        if ($_POST) {
            if (!get_permission('frontend_prstoriess', 'is_add')) {
                access_denied();
            }
            ...
            if ($this->form_validation->run() == false) {
                ...
                $this->prstoriess_model->save($data);
                set_alert('success', translate('information_has_been_saved_successfully'));
                redirect(base_url('frontend/prstoriess'));
            }
        }
        ...
        $this->data['testimoniallist'] = $this->prstoriess_model->get_list('front_cms_prstoriess');
        $this->data['title'] = translate('frontend');
        $this->data['sub_page'] = 'frontend/prstoriess';
    ...
    {
        if (!get_permission('frontend_prstoriess', 'is_edit')) {
            access_denied();
        }
        if ($_POST) {
            $this->slider_validation();
        ...
        $this->data['testimonial'] = $this->prstoriess_model->get_list('front_cms_prstoriess', array('id' => $id), true);
        $this->data['title'] = translate('frontend');
        $this->data['sub_page'] = 'frontend/prstoriess_edit';
        $this->data['main_menu'] = 'frontend';
        $this->load->view('layout/index', $this->data);
    }
    ...
    {
        if (!get_permission('frontend_prstoriess', 'is_delete')) {
            access_denied();
        }
        $image = $this->db->get_where('front_cms_prstoriess', array('id' => $id))->row()->image;
        if ($this->db->where(array('id' => $id))->delete("front_cms_prstoriess")) {
    ...
    {
        $this->form_validation->set_rules('patient_name', 'Prstories Title', 'trim|required|xss_clean');
