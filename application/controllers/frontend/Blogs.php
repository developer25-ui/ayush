...
class Blogs extends Admin_Controller
{
    ...
    public function index()
    {
        ...
        if (!get_permission('frontend_blogs', 'is_view')) {
            access_denied();
        }
         if($this->input->post('options')){
            $seo_content['page_title'] = $this->input->post('page_title');
                ...
                $this->db->where('id', 1);
                $this->db->update('front_cms_blogs_seo', $arraySeo);
                set_alert('success', translate('information_has_been_saved_successfully'));
                redirect(base_url('frontend/blogs'));
        }
        if ($_POST) {
            if (!get_permission('frontend_blogs', 'is_add')) {
                access_denied();
            }
        ...
        $this->data['testimoniallist'] = $this->blogs_model->get_list('front_cms_blogs');
        $this->data['title'] = translate('frontend');
        $this->data['sub_page'] = 'frontend/blogs';
    ...
    {
        if (!get_permission('frontend_blogs', 'is_edit')) {
            access_denied();
        }
        if ($_POST) {
            $this->slider_validation();
        ...
        $this->data['testimonial'] = $this->blogs_model->get_list('front_cms_blogs', array('id' => $id), true);
        $this->data['title'] = translate('frontend');
        $this->data['sub_page'] = 'frontend/blogs_edit';
        $this->data['main_menu'] = 'frontend';
        $this->load->view('layout/index', $this->data);
    }
    ...
    {
        if (!get_permission('frontend_blogs', 'is_delete')) {
            access_denied();
        }
        $image = $this->db->get_where('front_cms_blogs', array('id' => $id))->row()->image;
        if ($this->db->where(array('id' => $id))->delete("front_cms_blogs")) {
