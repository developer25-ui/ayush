...
class Events extends Admin_Controller
{
    ...
    public function index()
    {
        ...
        if (!get_permission('frontend_events', 'is_view')) {
            access_denied();
        }
        if($this->input->post('options')){
            $seo_content['page_title'] = $this->input->post('page_title');
                ...
                $this->db->where('id', 1);
                $this->db->update('front_cms_events_seo', $arraySeo);
                set_alert('success', translate('information_has_been_saved_successfully'));
                redirect(base_url('frontend/events'));
        }
        if ($_POST) {
            if (!get_permission('frontend_events', 'is_add')) {
                access_denied();
            }
        ...
        $this->data['testimoniallist'] = $this->events_model->get_list('front_cms_events');
        $this->data['title'] = translate('frontend');
        $this->data['sub_page'] = 'frontend/events';
    ...
    {
        if (!get_permission('frontend_events', 'is_edit')) {
            access_denied();
        }
        if ($_POST) {
            // $this->slider_validation();
        ...
        $this->data['testimonial'] = $this->events_model->get_list('front_cms_events', array('id' => $id), true);
        $this->data['title'] = translate('frontend');
        $this->data['sub_page'] = 'frontend/events_edit';
        $this->data['main_menu'] = 'frontend';
        $this->load->view('layout/index', $this->data);
    }
    ...
    {
        if (!get_permission('frontend_events', 'is_delete')) {
            access_denied();
        }
        $image = $this->db->get_where('front_cms_events', array('id' => $id))->row()->image;
        if ($this->db->where(array('id' => $id))->delete("front_cms_events")) {
    ...
    {
        $this->form_validation->set_rules('patient_name', 'Blog Title', 'trim|required|xss_clean');
    ...
    public function remove($id,$event_id)
    {
        $this->db->where(array('id' => $id))->delete("event_images");
        set_alert('success', translate('information_has_been_updated_successfully'));
        redirect(base_url('frontend/events/edit/'.$event_id));
