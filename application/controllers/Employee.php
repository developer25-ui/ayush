        ...
        if (get_permission('employee', 'is_edit')) {
            ...
            if ($this->form_validation->run() !== false) {
                ...
                if (isset($_FILES["document_file"]) && !empty($_FILES['document_file']['name'])) {
                    $config['upload_path'] = './uploads/attachments/documents/';
                    $config['allowed_types'] = 'gif|jpg|png|pdf|docx|webp|csv|txt';
                    $config['max_size'] = '2048';
                    $config['encrypt_name'] = true;
                    $this->upload->initialize($config);
                    if ($this->upload->do_upload("document_file")) {
        ...
        if ($_POST) {
            ...
            // die;
               ...
               if (isset($_FILES['image']) && $_FILES['image']['name'] != "") {
                    ...
                    move_uploaded_file($_FILES['image']['tmp_name'], 'uploads/images/chairman/'.$filename);
                    $data['image'] = 'uploads/images/chairman/'.$filename;
                }
                $this->db->where('id',1);
                $this->db->update('chairman',$data);
                
                set_alert('success', translate('information_has_been_saved_successfully'));
                redirect(base_url('employee/chairman'));
...
$this->data['headerelements'] = array(
        ...
        $this->data['title'] = translate('chairman');
        $this->data['content'] = $this->db->get_where('chairman',array('id=>1'))->row_array();
        $this->data['sub_page'] = 'employee/chairman';
        $this->data['main_menu'] = 'employee';
        $this->load->view('layout/index', $this->data);
    }
}
